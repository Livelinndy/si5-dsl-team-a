...
def process(line):
    ...
    regexes = {
        ...
        'add_title': re.compile('add\s+title\s+"(?P<text_content>[^"]+)"\s+to\s+(?P<clip_name>[a-zA-Z0-9]+)(\s+at\s+(?P<begin>[0-9]*h?[0-9]*m?[0-9]+s))?(\s+during\s+(?P<duration>[0-9]*h?[0-9]*m?[0-9]+s))?'),
        'add_subtitle': re.compile('add\s+subtitle\s+"(?P<text_content>[^"]+)"\s+to\s+(?P<clip_name>[a-zA-Z0-9]+)(\s+at\s+(?P<begin>[0-9]*h?[0-9]*m?[0-9]+s))?(\s+during\s+(?P<duration>[0-9]*h?[0-9]*m?[0-9]+s))?'),
    ...
    if regex_key == 'add_text':
        ...
        return AddText(clip_name, duration, text_content, begin, rgb_color, font_size)
    ...
    if regex_key == 'add_title':
        ...
        clip_name = match.group('clip_name')
        begin = match.group('begin')
        return AddText(clip_name, duration, text_content, begin, 'red', 40)

    if regex_key == 'add_subtitle':
        duration = match.group('duration')
        text_content = match.group('text_content')
        clip_name = match.group('clip_name')
        begin = match.group('begin')
        return AddText(clip_name, duration, text_content, begin, 'red', 20, pos_y = 'bottom')
